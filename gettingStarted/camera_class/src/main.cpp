...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (unsigned int i {}; i < 10; i++)
        {
            ...
            model = glm::rotate(model, glm::radians(cubeAngles[i]), glm::vec3(0.5f, 1.0f, 0.0f));
...
void processInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
        myCamera.ProcessKeyboard(Camera::CameraMovement::FORWARD, deltaTime);
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
        myCamera.ProcessKeyboard(Camera::CameraMovement::BACKWARD, deltaTime);
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
        myCamera.ProcessKeyboard(Camera::CameraMovement::LEFT, deltaTime);
    if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
        myCamera.ProcessKeyboard(Camera::CameraMovement::RIGHT, deltaTime);

    for (int i = 0; i <= 9; i++)
    {
        int glfwKey = GLFW_KEY_0 + i;   // maps 0→KEY_0, 1→KEY_1, ..., 9→KEY_9
        int state = glfwGetKey(window, glfwKey);

        if (state == GLFW_PRESS && lastState[glfwKey] == GLFW_RELEASE)
        {
            keys[i] = !keys[i]; // toggle cube i
            std::cout << "Toggled cube " << i << " to " << keys[i] << "\n";
        }
